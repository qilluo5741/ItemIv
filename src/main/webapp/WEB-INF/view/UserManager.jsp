...
<html>
  ...
  <body>
  <button class="btn btn-primary btn-lg btn-info" data-toggle="modal" data-target="#myModal" id="addBtn" >创建用户</button>
		    		  ...
		    		  <tr>
				         ...
				         <td>
				         	<c:if test="${empty u.roleName}">
				         		未分配角色
				         		<button id="${u.userId}"  name="${u.roleId}" type="button" class="btn btn-info btn-xs updateRole_">分配</button>
				         	</c:if>
						...
						<td>${u.createTime}</td>
				        <td>
						<button id="${u.userId}" type="button" class="btn btn-warning deletePwd">删除用户</button> 
						<button id="${u.userId}" type="button" class="btn btn-danger resetPwd">重置密码 </button> 
	...
	<script type="text/javascript">
		$(function(){
			...
			$(".resetPwd").click(function(){
				...
				if(confirm("确认重置密码吗？重置密码后新密码是‘123456’。")){
					$.ajax({
						url:"permission/resetPwd.do",
						type:"POST",
						data:{"userId":userId},
						dataType:"json",
